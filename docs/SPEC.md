...
| `devtools_ref` | scaffold release tag (e.g. `v1.5.0`) | the `sdlc-devtools` git-dep pin in the generated `pyproject.toml`'s `devtools` extra — bumped per release so `copier update` re-renders one pin line (bd p99) |
...
| 15 | archmap architecture autoviz (ADVISORY / doc-gen) | OURS `archmap.py` on grimp | (mechanism — nodes/edges derived from `packages`, no config) | scan paths = `packages`. Regenerates a tiered, edge-counted, clickable mermaid mirror-tree under `docs/architecture/` (one doc per nesting tier; arrows carry the module→module import count = coupling weight; `Drill:` markdown links descend). Wired 3 ways (2vt.4): manual `nox -s archmap` regen, a pre-commit REGEN hook (formatter pattern), and a CI advisory `--check` (`continue-on-error`, `::warning::` on drift). DOC-GEN — visualizes structure, does NOT enforce it; import-linter stays the directional gate. `--check` flags a stale/missing/orphan committed doc; drill rides markdown links not mermaid `click` (GitHub CSP blocks click nav). Graduates to a gate per the creep-to-gate program (2vt) |
