...
| 18 | law of demeter (ENFORCED) | OURS `demeter.py` `Demeter` | `[tool.structure] demeter_max_depth` (default 2) | Blocks a CALL that reaches THROUGH a field into a stranger: `self.store.get(k)` is 2 hops (reach your own field, then talk to it) and passes; `self.store.config.reload()` is 3 and fails — this class is now coupled to a type it never declared. **Only chains ending in a call count (bd v3c.5).** The gate originally counted attribute hops and never inspected `ast.Call`, which is the mechanism-instead-of-concept error: measured on a consumer, 20 of 22 findings were reads of a nested config tree (`cfg.generator.synth.bg.mode`) — no stranger, no method, no undeclared coupling — and the only way to silence one was a delegation wrapper, i.e. trading a false positive for Fowler's MIDDLE MAN. After the correction that consumer goes 22 → 2 and both survivors are genuine. Rests on gate 23: a `@property` is a method call spelled as attribute access, so "an attribute chain is only data" holds exactly as far as properties are pure. NOT counted: chains rooted at an imported MODULE (`np.linalg.norm`) or an imported CLASS (`Path.home()`) — a dotted namespace path, not a walk across objects; nor chains rooted in a call (no name to attribute it to). Reported once per chain, not per prefix, and the finding names the FRIEND to ask (`self._client`) rather than the root. The one architecture smell needing no graph — it is visible inside a single expression (R1). Raise the ceiling for delegation-heavy trees |
...
| 22 | feature envy (ENFORCED) | OURS `envy.py` `FeatureEnvy` | `[tool.structure] feature_envy_min` (default 4) | Fowler's smell at the granularity it lives — the METHOD. `arrows.py` emits per CLASS, which is right for "does this class depend on that one" but blind here: a class can be perfectly coupled while ONE of its methods belongs elsewhere. (`calls.py` now carries both method endpoints itself — bd f1u.2 — so the per-method cut is shared rather than recomputed.) Counts, per method, members touched on its OWN object versus on a SINGLE foreign class — spreading calls across several collaborators is orchestration, a different (often correct) shape, so envy is judged against one class only. Just the OUTERMOST link of a chain counts, so `self._store.get(k)` is one access to Store rather than also one to self. Receivers resolve as in `calls.py` (field type / param annotation / local constructor); an unresolvable receiver is not counted, so the ratio is computed only over accesses we understand. VALUE OBJECTS ARE NEVER THE TARGET: a SATELLITE (dataclass/enum/error, per gate 5c) is excluded, because reading a value object field-by-field is what it is FOR and the implied fix (move the method onto it) is usually impossible. `feature_envy_min` is the floor below which the ratio is noise — raise it for delegator-heavy repos (mappers/serializers/visitors) rather than weakening the rule |
| 23 | property purity (ENFORCED) | OURS `purity.py` `PropertyPurity` | (no config) | Blocks a `@property` whose body assigns to `self` — a read dressed as an attribute has to BE a read, or every reader silently becomes a writer: a debugger watch, a log line, a test assertion, an `if` that short-circuits. Ruff has no rule for it; its six property rules are PLR0206 (no parameters), RUF066 (no return) and B009/B010/B043 (constant getattr/setattr/delattr). All four write shapes count — `Assign`, `AugAssign`, `AnnAssign`, `Delete` — anywhere in the body including nested functions. `@x.setter`/`@x.deleter` are the DECLARED exception and exempt; `@cached_property` is held to the same bar, since its caching is the descriptor's job and a body that also writes is a second hand-rolled cache. The lazy-memo shape is what it catches most, so the message names `functools.cached_property` rather than only refusing. Precise but incomplete: a property calling a method that mutates elsewhere is out of scope, and proving purity in general is not attempted. MEASURED at 0 on all three consumer packages, and its first run on devtools itself found one (`Batch.resolver`, a hand-rolled memo, since converted) — so it BLOCKS from day one. It is also the premise gate 18 rests on |
| 24 | dynamic attribute access (ADVISORY) | vendored ast-grep + our `sg-rules/py-dynamic-attr.yml` | rule yml | Flags `getattr`/`setattr`/`hasattr` with a COMPUTED name — the case where pyrefly cannot check the access, the arch graph cannot resolve the edge, and vulture cannot see the attribute is used, while every gate stays green. Only the non-literal half: the constant-name form is already ruff B009/B010 (rewrite as `x.name`), and two gates saying one thing is worse than one. Escape hatch is the RULE-NAMED ignore (`# ast-grep-ignore: py-dynamic-attr`), never a blanket suppression, matching the PGH003/PGH004 stance on type-ignore and noqa. ADVISORY, and the measurement is the reason: devtools itself is at 0 — the interesting number, since it is the reflection-heavy tree and drives engines by name via `inspect.signature` + a Protocol cast rather than reflection — but the three consumer packages are at 9 / 3 / 11 and most are honest shapes (argparse `getattr(args, name)` plumbing, dotted-path config override, `getattr(torchvision.models, backbone)` model-zoo lookup, `setattr(module, name, LoraLinear(...))` torch surgery). Graduates to `error` when a real tree reaches zero, per repo, by replacing what has a typed form and marking what does not. Tests are out of scope by SCOPE not exception — ast-grep scans the root packages, and pytest's `monkeypatch.setattr` plus contract-checking `hasattr` are honest there |
...
test_layout = "bare"     # test layout: "bare" (path-mirror, no prefix — needs python_files=["*.py"]) |
                         # "mirror" (prefixed path-mirror) | "flat" (anywhere under tests/) | "off"
```
