"""Unit tests for devtools/tools/config.py — the packaged-config locator (ast-grep / jscpd paths).
...
import devtools
from devtools.tools import config
from devtools.tools.config import Config
...
def test_path(tmp_path):
    ...
    # pointing the PACKAGE ROOT at an empty directory rather than by deleting anything real. Config resolves
    # against `devtools.__file__` (not its own), so `config` can live in tools/ while the data stays at root.
    monkey = pytest.MonkeyPatch()
    monkey.setattr(devtools, "__file__", str(tmp_path / "__init__.py"))
